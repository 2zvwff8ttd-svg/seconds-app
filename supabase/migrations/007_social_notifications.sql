-- =============================================================================
-- Social notifications: schema, triggers, realtime
-- Run after notifications table exists:
--   supabase/sql/create-notifications-table.sql  OR  003_scheduled_publish.sql
-- =============================================================================
...
begin
  if not exists (
    ...
    raise exception 'Run create-notifications-table.sql first.';
  end if;
end $$;
...
begin
  ...
  loop
    ...
    where lower(p.username) = v_mention.uname
    limit 1;

    if v_mentioned_id is null then
      continue;
    end if;

    -- 動画オーナーへのコメント通知と重複させない
    if v_mentioned_id = v_owner then
      continue;
    end if;
