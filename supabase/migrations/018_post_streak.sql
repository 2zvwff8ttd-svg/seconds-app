-- 連続投稿ストリーク + 10日ごとの撮影時間ボーナス（5〜60秒）
...
comment on column public.profiles.current_streak is
  '投稿日（7時区切り）ベースの連続投稿日数。途切れたら1から再開';
comment on column public.profiles.last_post_date is
  '最後に投稿した投稿日（date）。同日の二重加算防止に使用';
...
-- current_streak が 10 の倍数（10,20,30…）の日は 5..60、それ以外は 5..30
