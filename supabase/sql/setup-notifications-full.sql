-- =============================================================================
-- 通知機能セットアップ（SQL Editor で 1 回だけ実行）
--
-- 含む内容:
--   ステップ 1: notifications テーブル + RLS
--   ステップ 2: ソーシャル通知トリガー + Realtime
--
-- 前提: 001_initial_schema.sql 適用済み（profiles, videos, likes, comments, follows）
-- =============================================================================
...
begin
  if not exists (
    ...
    raise exception 'public.profiles がありません。先に 001_initial_schema.sql を実行してください。';
  end if;
end $$;
...
begin
  if not exists (
    ...
    raise exception 'public.likes がありません。先に 001_initial_schema.sql を実行してください。';
  end if;
end $$;
...
where n.nspname = 'public'
  ...
  and tgname in ('trg_notify_on_like', 'trg_notify_on_follow', 'trg_notify_on_comment');
