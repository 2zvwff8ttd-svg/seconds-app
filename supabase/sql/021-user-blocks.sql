-- Supabase SQL Editor: run this once to add user blocks (Phase 1 UGC moderation).
-- Same as supabase/migrations/021_user_blocks.sql
...
-- Reject DM when either party has blocked the other
...
-- Verify:
-- select tablename from pg_tables where schemaname = 'public' and tablename = 'user_blocks';
-- select proname from pg_proc join pg_namespace n on n.oid = pronamespace
--   where n.nspname = 'public' and proname in ('block_user', 'unblock_user', 'is_blocked_with');
